...
>({
  ...
  const [selectedExistingConditionId, setSelectedExistingConditionId] =
    useState<string>();
  ...
  const selectableExistingConditions = useMemo(() => {
    ...
    const usedConditionIds = new Set(
      qualifyConditions.map((condition) => condition.id).filter(Boolean)
    );
    return availableQualifyConditions.filter(
      (condition) => !usedConditionIds.has(condition.id)
    );
  }, [availableQualifyConditions, qualifyConditions]);
  ...
  const handleAddExistingQualifyCondition = () => {
    if (!selectedExistingConditionId || !onAddExistingQualifyCondition) {
      return;
    }
    setQualifyConditionTabIndex(qualifyConditions.length);
    onAddExistingQualifyCondition(selectedExistingConditionId);
    setSelectedExistingConditionId(undefined);
    ensureQualifyAccordionOpen();
  };
          ...
          >
            <AccordionPrimitive.Header className="flex items-center gap-3 px-6 py-4">
              <AccordionPrimitive.Trigger
                ...
                className="focus-visible:border-ring focus-visible:ring-ring/50 flex min-w-0 flex-1 rounded-md text-left outline-none transition-all focus-visible:ring-[3px] disabled:pointer-events-none disabled:opacity-50"
              >
              ...
              <div className="hidden items-center gap-2 md:flex">
                ...
                selectableExistingConditions.length > 0 ? (
                  <>
                    <Select
                      value={selectedExistingConditionId}
                      onValueChange={setSelectedExistingConditionId}
                    >
                      <SelectTrigger className="w-[260px]">
                        <SelectValue placeholder="Seleccionar existente" />
                      </SelectTrigger>
                      <SelectContent>
                        {selectableExistingConditions.map((condition) => {
                          const typeLabel = getLabel(
                            qualifyConditionTypeOptions,
                            condition.type
                          );
                          const label = condition.description
                            ? `${typeLabel} - ${condition.description}`
                            : typeLabel;
                          return (
                    ...
                    <Button
                      ...
                      variant="secondary"
                      size="sm"
                      disabled={!selectedExistingConditionId}
                      onClick={handleAddExistingQualifyCondition}
                    >
              ...
              <AccordionPrimitive.Trigger
                disabled={qualifyConditions.length === 0}
                className="focus-visible:border-ring focus-visible:ring-ring/50 rounded-md p-1 outline-none transition-all focus-visible:ring-[3px] disabled:pointer-events-none disabled:opacity-50 [&[data-state=open]>svg]:rotate-180"
              >
