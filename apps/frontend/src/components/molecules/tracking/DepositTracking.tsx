...
export function DepositTracking({ conditionServerData }: DepositTrackingProps) {
  ...
  const qualifyingDeposit = tracking
    ? tracking.participatingDeposits.find(
        (deposit) => deposit.depositId === tracking.qualifyingDepositId
      ) ?? tracking.participatingDeposits[0]
    : undefined;
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-white rounded-md border p-3">
                ...
                <div className="font-mono text-sm font-semibold">
                  {tracking.totalDepositedAmount
                    ? formatCurrency(tracking.totalDepositedAmount)
                    : "N/A"}
                </div>
              </div>
            </div>

            {qualifyingDeposit?.code && (
              <div className="bg-white rounded-md border p-3">
                <div className="text-xs text-muted-foreground">Codigo Usado</div>
                <div className="font-mono text-sm">{qualifyingDeposit.code}</div>
              </div>
            )}

            {qualifyingDeposit?.depositedAt && (
              <div className="bg-white rounded-md border p-3">
                <div className="text-xs text-muted-foreground">Fecha de Deposito</div>
                <div className="font-mono text-sm">
                  {formatDateTime(qualifyingDeposit.depositedAt)}
                </div>
              </div>
            )}
          </div>
